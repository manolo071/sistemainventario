...
class Purchase extends Model
{
    ...
    { 
        return $this->leftJoin('suppliers', 'purch_orders.supplier_id', '=', 'suppliers.supplier_id')
                    ...
                    ->addSelect(DB::raw('SUM(purch_order_details.qty_invoiced) as total_qty'), DB::raw('SUM(purch_order_details.qty_received) as received_total'))
                    ->groupBy('purch_order_details.order_no')
    ...
    public function getAllPurchReceived(){
        $data = DB::table('receive_orders')
              ->leftJoin('receive_order_details','receive_order_details.receive_id','=','receive_orders.id')
              ->leftJoin('suppliers','suppliers.supplier_id','=','receive_orders.supplier_id')
              ->select('receive_orders.reference','suppliers.supp_name','suppliers.supplier_id','receive_orders.id as receive_no','receive_orders.order_no','receive_orders.receive_date',DB::raw('SUM(receive_order_details.quantity) as receive_qty'))
              ->groupBy('receive_order_details.receive_id')
              ->orderBy('receive_orders.id','desc')
              ->get();
        return $data;        
    } 
    
    public function getAllPurchReceivedFilter($from, $to, $item, $supplier){
        ...
        if($item != NULL && $supplier != NULL){    
         $data = DB::table('receive_orders')
                  ...
                  ->select('receive_orders.reference','suppliers.supp_name','suppliers.supplier_id','receive_orders.id as receive_no','receive_orders.order_no','receive_orders.receive_date',DB::raw('SUM(receive_order_details.quantity) as receive_qty'))
                  ->groupBy('receive_order_details.receive_id')
                  ->whereDate('receive_orders.receive_date','>=',$from)
                  ->whereDate('receive_orders.receive_date','<=',$to)
                  ->where('receive_order_details.item_code',$item)
                  ->where('receive_orders.supplier_id',$supplier)
                  ->orderBy('receive_orders.id','desc')
                  ->get();
           }elseif ($item == NULL && $supplier != NULL) {
            $data = DB::table('receive_orders')
                  ->leftJoin('receive_order_details','receive_order_details.receive_id','=','receive_orders.id')
                  ->leftJoin('suppliers','suppliers.supplier_id','=','receive_orders.supplier_id')
                  ->select('receive_orders.reference','suppliers.supp_name','suppliers.supplier_id','receive_orders.id as receive_no','receive_orders.order_no','receive_orders.receive_date',DB::raw('SUM(receive_order_details.quantity) as receive_qty'))
