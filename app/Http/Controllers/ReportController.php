    ...
    */
    public function inventoryStockOnHandPdf(){
        ...
        if(isset($_GET)) {
            ...
            $data['itemList'] = $itemList = $this->report->getInventoryStockOnHandPdf($type,$location); 
     ...
     public function salesReport(){
      ...
      //d($list,1);
        ...
        foreach ($list as $key => $value) {
          ...
          $sale['sale'][$key] = round(($value->sale_price_excl_tax),2);
          $cost['purchase'][$key] = round($value->purchase_price_incl_tax,2);
          $profit['profit'][$key] = round(($value->sale_price_excl_tax-$value->purchase_price_incl_tax),2);
...
//d($itemList,1);
        foreach($itemList as $key => $value){
            ...
            }elseif($type == 'yearly')
            {
              if($year=='all'){
                ...
                $datas[$key]['No Of Invoice'] = $value->total_order;
                $datas[$key]['Sales Volume'] = $value->qty;
                $datas[$key]['Sales Value'] = round($value->sale_price_excl_tax,2);
                $datas[$key]['Cost Value'] = round($value->purchase_price_incl_tax,2);
                $datas[$key]['Profit'] = round($value->sale_price_excl_tax-$value->purchase_price_incl_tax,2);
            }
    ...
    */
    public function salesReportByDateCsv($date){
        ...
        foreach ($itemList as $key => $item) {
                ...
                $data[$key]['Invoice No'] = $item->reference;
                $data[$key]['Date'] = formatDate($item->ord_date);
                $data[$key]['Customer'] = $item->name;
                $data[$key]['Qty'] = $item->quantity;
                $data[$key]['Sales Value('.Session::get('currency_symbol').')'] = $item->sale_price_excl_tax;
                $data[$key]['Cost Value('.Session::get('currency_symbol').')'] = $item->purchase_price_incl_tax;
                $data[$key]['Tax('.Session::get('currency_symbol').')'] = $item->sale_price_incl_tax-$item->sale_price_excl_tax;
    ...
    */
    public function salesReportByDatePdf($date){
        ...
        return $pdf->download('sales_report_by_date.pdf',array("Attachment"=>0)); 
...
//d($itemList,1);
        ...
        return Excel::create('sales_history_report', function($excel) use ($data) {
            $excel->sheet('mySheet', function($sheet) use ($data)
            {
                $sheet->fromArray($data);
            }); 
