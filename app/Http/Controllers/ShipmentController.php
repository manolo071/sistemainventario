...
class ShipmentController extends Controller
{
    ...
    */
    public function shipmentDetails($order_no,$shipment_id){
      ...
      $data['customerInfo']  = DB::table('sales_orders')
                             ...
                             ->select('debtors_master.name','debtors_master.phone','debtors_master.email','debtors_master.vat_no','cust_branch.br_name','cust_branch.br_address','cust_branch.shipping_street','cust_branch.shipping_city','cust_branch.shipping_state','cust_branch.shipping_zip_code','countries.country','cust_branch.shipping_country_id')
                             ->first();
    ...
    */
    public function sendShipmentInformationByEmail(Request $request){
        ...
        $shipmentId  = $request['shipment_id'];
        $invoiceName = 'shipment_'.time().'.pdf';
        if(isset($request['shipment_pdf']) && $request['shipment_pdf']=='on'){
            $this->shipmentPdfEmail($orderNo,$shipmentId,$invoiceName);
    ...
    public  function shipmentPdfEmail($orderNo,$shipmentId,$invoiceName){
       $data['taxInfo'] = $this->shipment->calculateTaxForDetail($shipmentId);
      $data['shipmentItem'] = DB::table('shipment')
                             ->where('shipment.id',$shipmentId)
                             ->leftjoin('shipment_details','shipment_details.shipment_id','=','shipment.id')
