    ...
    {   $date = array();
       ...
       //d($salesHistory,1);
        ...
        foreach ($salesHistory as $key => $history) {
            $totalSale += $history->sale_price_excl_tax;
            $totalCost += $history->purchase_price_incl_tax;
            $totalSoldQty += $history->quantity;
            $tax += ($history->sale_price_incl_tax-$history->sale_price_excl_tax);
        }
